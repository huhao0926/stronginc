...
//          std::cout<<FLAGS_vfile<<std::endl;
  ...
  FLAGS_efile = "../data/synmtic.e";
  srand( (unsigned)time(0));
  std::string base_yago_inc_filename1 ="../data/incsynmtic/add_e";
  std::string base_yago_inc_filename2 ="../data/incsynmtic/rm_e";
...
//    return true;
 ...
 void load_graph(Graph &dgraph,std::string v_file,std::string e_file, std::string basefilename1,std::string basefilename2,std::set<std::pair<VertexID,VertexID>> &exist_edges,int j)
 {
  ...
  GraphLoader dgraph_loader,qgraph_loader;
  srand( (unsigned)time(0));
  std::string basefilename1 ="../data/incsynmtic/add_e";
  std::string basefilename2 ="../data/incsynmtic/rm_e";
  int index = 1;
...
//  std::cout<<d_Q<<std::endl;
  ...
  while(i<10){
     ...
     while (true){
         generate_random_add_edge(dgraph_num_vertices,max_dual_set,affectted_center_node,exist_edges,500,add_e_set);
         if (add_is_valid(dgraph,qgraph,max_dual_set,add_e_set)){
     ...
     while(true){
        generate_random_remove_edge(dgraph_num_vertices,max_dual_set,affectted_center_node,exist_edges,500,remove_e_set);
         if(rm_is_valiad(dgraph,qgraph,max_dual_set,remove_e_set)){
...
//  FLAGS_efile = "../data/yago.edge";
  ...
  while (index <200){
      ...
      int j=1;
      while (j<20){
          std::vector<std::pair<VertexID,VertexID>> add_edges,rm_edges;
  ...
  while (index <2){
     ...
     // dualsim.dual_simulation(dgraph,qgraph,sim,initialized_sim);
      ...
      //   std::cout<<"calculate inc strong"<<(float)(end2-start2)/CLOCKS_PER_SEC<<"s"<<std::endl;
          std::cout<<j<<' '<<(float)(end1-start1)/CLOCKS_PER_SEC<<' '<<(float)(end2-start2)/CLOCKS_PER_SEC<<std::endl;
         //std::cout<<strongr_issame(qgraph,direct_strong,tmp_r)<<std::endl;
          j+=1;
      }
      index +=1;
   }
